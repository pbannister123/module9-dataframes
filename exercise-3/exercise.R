...
expendiatures <- data.frame(USPersonalExpenditure)
# What are the column names of your dataframe?

colnames(expendiatures)
...
rownames(expendiatures)

# Create a column "category" that is equal to your rownames

expendiatures$category <- rownames(expendiatures)
...
expendiatures["Food and Tobacco", "X1960"]

# What was the highest expenditure category in 1960?
# Hint: use the `max()` function to find the largest, then compare that to the column
max(expendiatures$X1960)
expendiatures$category[expendiatures$X1960 == max(expendiatures$X1960)]
